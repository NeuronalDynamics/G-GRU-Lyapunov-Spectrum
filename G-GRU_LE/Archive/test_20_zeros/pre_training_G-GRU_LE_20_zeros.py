...
def critical_ggru_init(model: GGRUSMNIST, g_star, k, scheme="orthogonal"):
    ...
    Because var(W_hh) = (A² + (k-1)B²)/k, setting A=B gives σ² = B².
    We therefore scale A=B with  g_star*sqrt(k)/sqrt(H).
    """
...
def lambda_max(net: nn.Module,
    ...
    for t in range(warm + T):
        ...
        if t < warm:
            continue
        J = rnn_jacobian_autograd(cell, driver[t], h)
        v = J @ q
        r = v.norm()
        q = v / (r + 1e-12)
...
def main():
    ...
    ap.add_argument('--gain', type=float, default=3.58,
                help='initialisation gain g (ignored when --calibrate)') #4.0 for GRU #3.5 for GGRU k=4 #2.2 for GGRU k=8
    ...
    if args.calibrate:
        if args.model == 'ggru':
            g_grid = np.arange(2.15, 2.25, 0.01)
        else:
