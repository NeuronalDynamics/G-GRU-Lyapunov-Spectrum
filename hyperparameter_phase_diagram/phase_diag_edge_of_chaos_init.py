...
# ================================================================
"""
python phase_diag.py --phase --p-grid 2,4,6,8,10,15,20 --seeds 0
...
def init_edge_of_chaos(model: nn.Module) -> float:
    ...
    return p
'''
def init_edge_of_chaos(model, p_fixed=None):
    # ---------------------------------------------
    # 1) choose p
    p = float(p_fixed) if p_fixed is not None else \
        float(torch.empty(1).uniform_(0.1, 30))
    # ---------------------------------------------
    # 2) locate the *cell* object, independent of wrapper
    cell = model.gru if hasattr(model.gru, "weight_ih_l0") else model.gru.cell
    # ---------------------------------------------
    # 3) uniformly fill then orthogonalise
    if hasattr(cell, "weight_ih"):
        # GRUCell-style (baseline path)
        nn.init.uniform_(cell.weight_ih, -p, p)
        nn.init.uniform_(cell.weight_hh, -p, p)
        nn.init.orthogonal_(cell.weight_ih)
        nn.init.orthogonal_(cell.weight_hh)
        # gate-bias layout: [reset, update, new]
        H = cell.hidden_size
        with torch.no_grad():
            cell.bias_hh[:H]   = -p     # reset
            cell.bias_hh[H:2*H] = +p    # update
            cell.bias_hh[2*H:] = 0.0
            cell.bias_ih.zero_()
    else:
        # nn.GRU-style OR your PermEquiGRUCell
        for w in (cell.W_ir.weight, cell.W_iz.weight, cell.W_in.weight):
            nn.init.uniform_(w, -p, p)
            nn.init.orthogonal_(w)
        # α/β parameters already ∼𝒩(0,1); leave them
    return p
...
def phase_diagram(args):
    ...
    for seed, p_val, G in itertools.product(seeds, p_grid, groups):
        ...
        p_val = init_edge_of_chaos(net, p_fixed=p_val)      # fixed-p init
